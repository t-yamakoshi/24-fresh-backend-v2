...
import (
	...
	"github.com/t-yamakoshi/24-fresh-backend-v2/schema/db/ent/entgen"
)
...
func (f UserModelFunc) Mutate(ctx context.Context, m entgen.Mutation) (entgen.Value, error) {
	if mv, ok := m.(*entgen.UserModelMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *entgen.UserModelMutation", m)
}
